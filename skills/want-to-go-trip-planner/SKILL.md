...
python3 scripts/want_to_go.py scan --path want-to-go-trip-planner-skill-2.4.1.zip --mode package
```
...
```bash
python3 scripts/build_release.py --zip ../../dist/want-to-go-trip-planner-skill-2.4.1.zip
python3 scripts/build_release.py --verify --zip ../../dist/want-to-go-trip-planner-skill-2.4.1.zip
```

构建器同时生成 `.zip.sha256`。GitHub 的 `v2.4.1` 标签流程使用固定提交的 Actions 生成 provenance attestation；本地 checksum 证明文件完整性，远端 attestation 在标签流程实际成功后证明 GitHub 仓库与构建工作流来源。
